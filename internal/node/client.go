...
func MakeUserListen(config *Config) (*Client, error) {
	...
	user := User{Name: records[0][0], Senderstyle: records[0][1]}
	c := &Client{
		Name:        config.Username,
		Senderstyle: config.Color,
		Conn:        conn,
...
func MakeUserConnect(config *Config, host string) (*Client, error) {
	conn, err := net.Dial("tcp", host+config.Socket)
	if err != nil {
